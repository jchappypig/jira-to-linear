...
async function runMigration(opts) {
    ...
    if (!jiraBaseUrl || !jiraEmail || !jiraToken || !linearKey) {
        throw new Error("Missing env vars. Ensure JIRA_BASE_URL, JIRA_EMAIL, JIRA_API_TOKEN, and LINEAR_API_KEY are set in your .env file.");
    }
    const jiraClient = new jira_1.JiraClient(jiraBaseUrl, jiraEmail, jiraToken);
    const linearClient = new linear_1.LinearMigrationClient(linearKey);
    ...
    const sorted = (0, mapper_1.sortIssuesByHierarchy)(jiraIssues);
    // Collect all target Linear team IDs for label/state pre-loading
    const teamIds = new Set();
    for (const issue of sorted) {
        const jiraTeamName = issue.fields.customfield_10001?.name ?? issue.fields.project.name;
        const teamId = linearClient.resolveTeamId(jiraTeamName, config.teamMapping) ??
    ...
    if (teamIds.size === 0) {
        throw new Error("Could not resolve any Jira projects to Linear teams. Check your teamMapping in config.json.");
    }
    await linearClient.loadLabels([...teamIds]);
    await linearClient.loadWorkflowStates([...teamIds]);
    const mapper = new mapper_1.IssueMapper(config, linearClient, jiraBaseUrl);
    ...
    for (const jiraIssue of sorted) {
        ...
        const jiraTeamName = jiraIssue.fields.customfield_10001?.name ??
        ...
        try {
            ...
            for (const jiraComment of jiraComments) {
                const commentText = (0, adf_to_markdown_1.convertAdfToMarkdown)(jiraComment.body);
                if (!commentText.trim())
                    continue;
